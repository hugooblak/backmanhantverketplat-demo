...
add_filter(
	...
	function ( $exporters ) {
		$exporters['backmanhantverketplat-leads'] = array(
			'exporter_friendly_name' => __( 'Quote requests', 'backmanhantverketplat-leads' ),
			'callback'               => function ( $email ) {
				$items = array();
				foreach ( npl_leads_by_email( $email ) as $lead ) {
					$data = array( array( 'name' => __( 'Received', 'backmanhantverketplat-leads' ), 'value' => $lead->post_date ) );
					...
					$items[] = array(
						'group_id'    => 'backmanhantverketplat-leads',
						'group_label' => __( 'Quote requests', 'backmanhantverketplat-leads' ),
...
add_filter(
	...
	function ( $erasers ) {
		$erasers['backmanhantverketplat-leads'] = array(
			'eraser_friendly_name' => __( 'Quote requests', 'backmanhantverketplat-leads' ),
			'callback'             => function ( $email ) {
				$removed = false;
				foreach ( npl_leads_by_email( $email ) as $lead ) {
