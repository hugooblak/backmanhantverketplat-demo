...
add_action(
	...
	function () {
		register_post_type(
			...
			array(
				'labels'          => array(
					'name'          => __( 'Leads', 'backmanhantverketplat-leads' ),
					'singular_name' => __( 'Lead', 'backmanhantverketplat-leads' ),
					'edit_item'     => __( 'Förfrågan', 'backmanhantverketplat-leads' ),
					'search_items'  => __( 'Search leads', 'backmanhantverketplat-leads' ),
					'not_found'     => __( 'Inga förfrågningar än. Skicka formuläret så dyker den upp här.', 'backmanhantverketplat-leads' ),
					'all_items'     => __( 'Alla förfrågningar', 'backmanhantverketplat-leads' ),
				),
...
add_filter(
	...
	function () {
		return array(
			...
			'title'        => __( 'Name', 'backmanhantverketplat-leads' ),
			'npl_phone'    => __( 'Phone', 'backmanhantverketplat-leads' ),
			'npl_service'  => __( 'Needs', 'backmanhantverketplat-leads' ),
			'npl_timeline' => __( 'When', 'backmanhantverketplat-leads' ),
			'npl_zip'      => __( 'ZIP', 'backmanhantverketplat-leads' ),
			'npl_source'   => __( 'Source', 'backmanhantverketplat-leads' ),
			'npl_status'   => __( 'Status', 'backmanhantverketplat-leads' ),
			'date'         => __( 'Received', 'backmanhantverketplat-leads' ),
		);
	}
);
...
function npl_source_summary( $post_id ) {
	...
	return __( 'Direct', 'backmanhantverketplat-leads' );
}
...
add_action(
	...
	function () {
		add_meta_box( 'npl_details', __( 'What they told us', 'backmanhantverketplat-leads' ), 'npl_details_box', 'npl_lead', 'normal', 'high' );
		add_meta_box( 'npl_status', __( 'Status', 'backmanhantverketplat-leads' ), 'npl_status_box', 'npl_lead', 'side', 'high' );
	}
);
...
function npl_details_box( $post ) {
	$rows = array(
		__( 'Name', 'backmanhantverketplat-leads' )     => get_post_meta( $post->ID, 'name', true ),
		__( 'Phone', 'backmanhantverketplat-leads' )    => get_post_meta( $post->ID, 'phone', true ),
		__( 'E-post', 'backmanhantverketplat-leads' )    => get_post_meta( $post->ID, 'email', true ),
		__( 'ZIP', 'backmanhantverketplat-leads' )      => get_post_meta( $post->ID, 'zip', true ),
		__( 'Needs', 'backmanhantverketplat-leads' )    => npl_label( 'service', get_post_meta( $post->ID, 'service', true ) ),
		__( 'When', 'backmanhantverketplat-leads' )     => npl_label( 'timeline', get_post_meta( $post->ID, 'timeline', true ) ),
		__( 'Source', 'backmanhantverketplat-leads' )   => npl_source_summary( $post->ID ),
	);
	...
	$rows[ __( 'Samtycke lämnat', 'backmanhantverketplat-leads' ) ] = get_post_meta( $post->ID, 'consent_at', true ) . ' — "' . get_post_meta( $post->ID, 'consent_text', true ) . '"';
...
function npl_status_box( $post ) {
	...
	echo '<label class="screen-reader-text" for="npl-status">' . esc_html__( 'Status', 'backmanhantverketplat-leads' ) . '</label><select id="npl-status" name="npl_status" style="width:100%">';
