 ...
 * or photo. They call ba_lead() instead. To build a demo for a real firm, the
 ...
 *
 * With no lead-data.php present the defaults below render the fictional Bäckman Hantverket Plattsättning AB.
 *
 ...
 * @return string
 */
function ba_sample_note() {
	return ba_lead( 'ar_riktig' ) ? '' : ' <span class="ba-sample">(platshållare)</span>';
}
 ...
 * @return string File name for ba_lead_img().
 */
function ba_lead_bild( $i ) {
	$bilder = (array) ba_lead( 'bilder', array() );
 ...
 * @return string
 */
function ba_ring_label() {
	$namn = ba_lead( 'kontakt' );
	return $namn ? sprintf( 'Ring %s · %s', $namn, ba_lead( 'telefon' ) ) : 'Ring ' . ba_lead( 'telefon' );
}
